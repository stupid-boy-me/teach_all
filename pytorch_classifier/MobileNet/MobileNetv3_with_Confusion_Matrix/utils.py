...
def evaluate(model, data_loader, device):
    ...
    with torch.no_grad():   # 可以跟上面的@torch.no_grad()  替换掉
        for step, data in enumerate(data_loader):
            ...
            tp_fp_fn_tn = metric._process(pred,labels)
            accuracy = metric.accuracy()
            precision = metric.precision()
            recall = metric.recall()
    return sum_num.item() / total_num, metric,tp_fp_fn_tn,accuracy,precision,recall
...
# 针对混淆矩阵的输出制作成一个表格
def make_table(data,columns=["acc","tp_fp_fn_tn","accuracy","precision","recall"],):
    '''
    data的格式
    data = [[110,120,110],[130,130,130],[130,120,130]]  [[第一行数据],[第二行数据],[第三行数据]]
    所以此的格式是:data = [[acc,tp_fp_fn_tn,accuracy,precision,recall]]
    '''
    import pandas as pd
    ...
    '''
    set_cols_align()方法 设置所需的列的对齐方式，主要参数及说明如下：
                        l:左对齐。
                        c:居中对齐。
                        r:右对齐。
    set_cols_dtype()方法 该方法用于设置数据类型，主要参数及说明如下：
                        ...
                        i:作为整型。
    '''
    tb.set_cols_align(['c','c','c','c','c']) # 设置对齐方式
    tb.set_cols_dtype(['f','f','f','f','f']) # 设置每列的数据类型为整型
