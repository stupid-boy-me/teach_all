...
from torch.utils.tensorboard import SummaryWriter
from utils import data_set_split,adjust_learning_rate,train_one_epoch,evaluate,plot_confusion_matrix,make_table
...
parser.add_argument("--Save_Confusion_Matrix_folder",default="/algdata01/yiguo.huang/hyg_deep_learning_me/MobileNet/MobileNetv3_with_Confusion_Matrix/Confusion_Matrix_picture//",help='Dataset directory path')
parser.add_argument("--Save_weights_folder",default="/algdata01/yiguo.huang/hyg_deep_learning_me/MobileNet/MobileNetv3_with_Confusion_Matrix/weights_folder//",help='target_data_folder directory path')
...
def main():
    ...
    for epoch in range(args.num_epochs):
        ...
        # validate
        acc,metric,tp_fp_fn_tn,accuracy,precision,recall = evaluate(model=create_net,
                                                                data_loader=val_loader,
                                                                device=device)
        '''Confusion Matrix 的一些基本操作如下'''
        ...
        data = [[acc,tp_fp_fn_tn,accuracy,precision,recall]]
        ...
        '''绘制曲线包含loss曲线 accuracy曲线 learning_rate曲线 P-R曲线'''
        tags = ["loss", "accuracy", "learning_rate","P-R"]
        tb_writer.add_scalar(tags[0], mean_loss, epoch)
        tb_writer.add_scalar(tags[1], acc, epoch)
        tb_writer.add_scalar(tags[2], optimizer.param_groups[0]["lr"], epoch)
        # tb_writer.add_pr_curve(tags[3],)
        if (epoch+1) % 5 == 0:
