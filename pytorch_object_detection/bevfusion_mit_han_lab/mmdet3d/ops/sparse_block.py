...
def _apply_bn(bn, features):
    """Apply BN; fall back to eval stats when only 1 sparse site (batch=1)."""
    if bn.training and features.shape[0] <= 1:
        bn.eval()
        out = bn(features)
        bn.train()
        return out
...
class SparseBottleneck(Bottleneck, spconv.SparseModule):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out.features = _apply_bn(self.bn1, out.features)
        out.features = self.relu(out.features)

        out = self.conv2(out)
        out.features = _apply_bn(self.bn2, out.features)
        out.features = self.relu(out.features)

        out = self.conv3(out)
        out.features = _apply_bn(self.bn3, out.features)
...
class SparseBasicBlock(BasicBlock, spconv.SparseModule):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out.features = _apply_bn(self.norm1, out.features)
        out.features = self.relu(out.features)

        out = self.conv2(out)
        out.features = _apply_bn(self.norm2, out.features)

        if self.downsample is not None:
            identity = self.downsample(x)

        if out.features.shape[0] == identity.shape[0]:
            out.features += identity
        out.features = self.relu(out.features)
