            ...
            if is_spconv_module(module):  # use SpConvTensor as input
                ...
                if isinstance(input, SparseConvTensor):
                    if input.indices.shape[0] != 0:
                        # BatchNorm needs >1 values in train mode; sparse tensors
                        # can temporarily have a single active site with batch=1.
                        if (
                            module.training
                            and isinstance(module, _BatchNorm)
                            and input.features.shape[0] <= 1
                        ):
                            module.eval()
