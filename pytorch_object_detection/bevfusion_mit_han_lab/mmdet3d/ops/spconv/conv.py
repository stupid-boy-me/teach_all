...
class SparseConvolution(SparseModule):
    ...
    def forward(self, input):
        ...
        if features.numel() == 0 or indices.numel() == 0:
            # Keep correct downsampled spatial shape for empty sparse tensors
            # (batch=1 edge cases); otherwise densify() returns wrong H/W.
            empty_inds = indices.new_zeros((0, self.ndim + 1))
            out_tensor = SparseConvTensor(
                features.new_zeros((0, self.out_channels)),
                empty_inds,
                out_spatial_shape,
                input.batch_size,
            )
        ...
        if outids is None or outids.numel() == 0 or outids.shape[0] == 0:
            empty_inds = indices.new_zeros((0, self.ndim + 1))
            out_tensor = SparseConvTensor(
                features.new_zeros((0, self.out_channels)),
                empty_inds,
