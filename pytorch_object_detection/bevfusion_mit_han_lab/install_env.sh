...
# ---------------------------------------------------------------------------
# 9) 下载官方预训练权重（约 865MB，供训练/评测/可视化直接使用）
# ---------------------------------------------------------------------------
NEED_CKPTS=(
...
missing=0
for f in "${NEED_CKPTS[@]}"; do
  [[ -f "$REPO_ROOT/pretrained/$f" ]] || missing=1
done
if [[ "$missing" -eq 1 ]]; then
  log "下载官方预训练权重到 pretrained/ （脚本: tools/download_pretrained.sh）"
  bash "$REPO_ROOT/tools/download_pretrained.sh"
else
  log "pretrained/ 关键权重已存在，跳过下载"
fi
